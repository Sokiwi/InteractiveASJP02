parseASJP <- function(fn, outfile_concat) {
	...
	if ( length(metadata1st) > 0 ) {
		names <- c(); wls_fam <- c(); wls_gen <- c(); e <- c(); hh <- c()
	...
	dbf <- data.frame(names, wls_fam, wls_gen, e, hh, lat, lon, pop, wcode, iso, stringsAsFactors=FALSE)
	#clean up
	rm(names); rm(wls_fam); rm(wls_gen); rm(e); rm(hh); rm(lat); rm(lon); rm(pop); rm(wcode); rm(iso)
	#extract wordlists
...
#		cat("Processing Swadesh list item ", i,"\n")
	...
	write.table(dbf,file=outfile_concat,row.names=FALSE,quote=FALSE,sep="\t")
}
